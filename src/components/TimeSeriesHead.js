...
const TimeSeriesHead = ({ handleOptions }) => {
   ...
   return (
      <header className="time-series-head">
         <h2>Spread Trends</h2>
         <div className={`time-series-range ${themeClass}`}>
            {TIMESERIES_RANGE.map((range, index) => {
               return (
                  <button
                     key={index}
                     className={
                        index === activeBtn ? "range-btn active" : "range-btn"
                     }
                     onClick={() => {
                        setActiveBtn(index)
                        handleOptions("range", range.value)
                     }}
                  >
                     {range.name}
                  </button>
         ...
         <select
            className={`state-list ${themeClass}`}
            value={selectedState}
            onChange={(e) => {
               setSelectedState(e.target.value)
               handleOptions("stateCode", e.target.value)
            }}
         >
